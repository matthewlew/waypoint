...
function initDrag(container,onDone){
  if(!container)return;
  let di=null,oi=null;
  const rows=()=>[...container.querySelectorAll(':scope>.dest-row,:scope>.et-dest-row,:scope>.card,:scope>.list-row')];
  rows().forEach((row,idx)=>{
    ...
    target.addEventListener('pointerup',()=>{
      ...
      if(oi!==null&&oi!==di){
        ...
        if(onDone)onDone(rows().map(r=>r.dataset.id || r.dataset.itemId).filter(Boolean));
        Haptic.success();
      }
      di=null;oi=null;
    });
...
function renderPack(){
  ...
  document.querySelectorAll('.items-drag-container').forEach(c=>initDrag(c, (newOrder) => {
...
function renderByCategory(){
  ...
  ordered.forEach(cat=>{
    ...
    h+=packCard({id:'cat-'+cat.id,gid:cat.id,name:cat.name,sub:cat.desc||'',
      ...
      body:`<div class="items-drag-container">${items.map(i=>itemRow(i)).join('')}</div>`+`<div class="row-add" onclick="openAddItem('cat','${cat.id}')">+ Add item</div>`});
  });
...
function renderByBag(){
  ...
  ordered.forEach(bag=>{
    ...
    h+=packCard({id:'bag-'+bag.id,gid:bag.id,name:bag.name,sub:bag.desc,
      ...
      body:`<div class="items-drag-container">${items.map(i=>itemRow(i,true)).join('')}</div>`+`<div class="row-add" onclick="openAddItem('bag','${bag.id}')">+ Add item</div>`});
  });
...
function renderByPurpose(){
  ...
  ordered.forEach(pur=>{
    ...
    h+=packCard({id:'pur-'+pur.id,gid:pur.id,name:pur.name,sub:'',
      ...
      body:`<div class="items-drag-container">${items.map(i=>itemRow(i)).join('')}</div>`+`<div class="row-add" onclick="openAddItem('pur','${pur.id}')">+ Add item</div>`});
  });
